...
resource "null_resource" "worker-ssl" {

  provisioner "local-exec" {
    command = <<LOCAL_EXEC
echo "${tls_locally_signed_cert.worker_cert.cert_pem}" > worker.pem
echo "${tls_private_key.worker_key.private_key_pem}" > worker-key.pem
LOCAL_EXEC
  }
}
