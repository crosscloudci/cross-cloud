...
resource "tls_cert_request" "kubelet_csr" {
  key_algorithm = "${tls_private_key.kubelet_key.algorithm}"
  private_key_pem = "${tls_private_key.kubelet_key.private_key_pem}"
  subject {
    common_name = "${var.tls_kubelet_cert_subject_common_name}"
...
resource "tls_locally_signed_cert" "kubelet_cert" {
  cert_request_pem = "${tls_cert_request.kubelet_csr.cert_request_pem}"
